...
var classMapping = map[string]func(*vm.VM) *vm.Class{
	...
	"Magnitude":       func(v *vm.VM) *vm.Class { return v.MagnitudeClass },
	"Number":          func(v *vm.VM) *vm.Class { return v.NumberClass },
	"SmallInteger":    func(v *vm.VM) *vm.Class { return v.SmallIntegerClass },
	"BigInteger":      func(v *vm.VM) *vm.Class { return v.BigIntegerClass },
	"Float":           func(v *vm.VM) *vm.Class { return v.FloatClass },
	"Collection":      func(v *vm.VM) *vm.Class { return v.CollectionClass },
		...
		for _, classDef := range pf.sf.Classes {
			...
			if ok {
				...
				if classDef.Superclass != "" && classDef.Superclass != "Object" {
					...
					} else if superVal, ok := vmInst.Global(classDef.Superclass); ok {
