## ArrayList (growable collection)

ArrayList is a Go-backed growable array with amortized O(1) append. Use it instead of Array when building collections incrementally — Array's `copyWith:` is O(n) per append (O(n²) total), while ArrayList's `add:` is O(1) amortized.

```smalltalk
"Creation"
list := ArrayList new.           "Empty, default capacity"
list := ArrayList new: 100.      "Empty with capacity hint"
list := ArrayList withAll: #(1 2 3).  "From existing Array"

"Adding (amortized O(1))"
list add: element.               "Append one element"
list addAll: #(4 5 6).           "Append all from Array or ArrayList"

"Access (same protocol as Array)"
list at: 0.                      "Indexed access"
list at: 0 put: value.           "Indexed set"
list first. list last.           "Endpoints"
list size. list capacity.        "Size info"
list includes: element.          "Membership"
list indexOf: element.           "Search"

"Removing"
list removeLast.                 "O(1) — remove and return last"
list removeAt: index.            "O(n) — shift remaining"
list clear.                      "Remove all, keep capacity"

"Iteration (same protocol as Array)"
list do: [:each | ...].          "Side effects"
list collect: [:x | x * 2].     "Map → returns ArrayList"
list select: [:x | x > 3].      "Filter → returns ArrayList"
list reject: [:x | x > 3].      "Inverse filter → returns ArrayList"
list detect: [:x | x > 3].      "Find first match"
list inject: 0 into: [:s :x | s + x].  "Fold/reduce"

"Conversion and copying"
list asArray.                    "Snapshot to fixed-size Array"
list copy.                       "Shallow copy → new ArrayList"
list sort: [:a :b | a - b].     "In-place sort"
```

ArrayList is non-serializable (like Channel, Mutex, etc.) — it cannot be sent across nodes. Use `asArray` first if you need to serialize.

Internally: NaN-boxed with marker `56 << 24`, backed by Go `[]Value` slice. Registered in `ConcurrencyRegistry`. Primitive methods in `vm/arraylist_primitives.go`, Maggie wrapper in `lib/ArrayList.mag`.
