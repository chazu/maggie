...
//
// CollectAllObjects builds a pointer-keyed object table and the encode phase
// later looks objects up by pointer. The custom collector that could once free
// an object mid-walk is gone (heap objects are Go-GC-traced and stay alive while
// the walk holds them), so no collector lock is needed here.
