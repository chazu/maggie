...
func TestDuckDBCreateInsertQuery(t *testing.T) {
	...
	nameKey := vmInst.Registry().NewStringValue("name")
		nameVal, exists := dictObj.Get(vmInst.Registry(), nameKey)
	if !exists {
...
func TestDuckDBUpdate(t *testing.T) {
	...
	valKey := vmInst.Registry().NewStringValue("val")
		val, _ := row.Get(vmInst.Registry(), valKey)
	if val.SmallInt() != 99 {
...
func TestDuckDBDelete(t *testing.T) {
	...
	nKey := vmInst.Registry().NewStringValue("n")
		n, _ := row.Get(vmInst.Registry(), nKey)
	if n.SmallInt() != 2 {
...
func TestDuckDBTypeMapping(t *testing.T) {
	...
	iVal, _ := row.Get(vmInst.Registry(), iKey)
	...
	dVal, _ := row.Get(vmInst.Registry(), dKey)
	if !dVal.IsFloat() {
	...
	vVal, _ := row.Get(vmInst.Registry(), vKey)
	...
	bVal, _ := row.Get(vmInst.Registry(), bKey)
	if bVal != vm.True {
	...
	// NULL -> Nil
	nKey := vmInst.Registry().NewStringValue("n")
	nVal, _ := row.Get(vmInst.Registry(), nKey)
	if nVal != vm.Nil {
...
func TestDuckDBParquetRoundTrip(t *testing.T) {
	...
	labelVal, _ := row0.Get(vmInst.Registry(), labelKey)
...
func TestDuckDBAppenderFlush(t *testing.T) {
	...
	n, _ = row3.Get(vmInst.Registry(), nKey)
	if n.SmallInt() != 6 {
