...
// ---------------------------------------------------------------------------
// CborImageReader: Reads and deserializes VM state from a CBOR image
// ---------------------------------------------------------------------------

// CborImageReader deserializes a CBOR-encoded image into a VM.
...
// NewCborImageReader creates a new CborImageReader from CBOR bytes.
// It unmarshals the top-level tagged envelope.
func NewCborImageReader(data []byte) (*CborImageReader, error) {
	var tag cbor.Tag
	if err := cbor.Unmarshal(data, &tag); err != nil {
		return nil, fmt.Errorf("CborImageReader: failed to unmarshal CBOR tag: %w", err)
	}
	if tag.Number != imgTagHeader {
		return nil, fmt.Errorf("CborImageReader: expected tag %d, got %d", imgTagHeader, tag.Number)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("CborImageReader: failed to re-encode envelope: %w", err)
	}

	var envelope cborImageEnvelope
	if err := cbor.Unmarshal(contentBytes, &envelope); err != nil {
		return nil, fmt.Errorf("CborImageReader: failed to decode envelope: %w", err)
	}

	return &CborImageReader{
		envelope:      envelope,
...
func (cr *CborImageReader) readMethod(vm *VM, def *cborMethodDef) (*CompiledMethod, error) {
	name := cr.resolveString(def.Name)
...
func (cr *CborImageReader) readBlock(vm *VM, def *cborBlockDef) (*BlockMethod, error) {
	// Decode literals
	literals := make([]Value, len(def.Literals))
	for j, raw := range def.Literals {
		val, err := decodeImageValue(vm, cr.decoder, raw)
...
func (cr *CborImageReader) resolveString(idx uint32) string {
