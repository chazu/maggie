...
func TestStringPrimConcat(t *testing.T) {
	...
	if GetStringContent(result) != "hello world" {
		t.Errorf("primConcat: returned %q, want %q", GetStringContent(result), "hello world")
	}
}
...
func TestStringPrimLessThan(t *testing.T) {
	...
	s1 := NewStringValue("abc")
	s2 := NewStringValue("abd")
	s3 := NewStringValue("abc")
...
func TestStringPrimIncludes(t *testing.T) {
	...
	s := NewStringValue("hello world")

	// Contains single char
	result := vm.Send(s, "primIncludes:", []Value{NewStringValue("o")})
	if result != True {
		t.Errorf("primIncludes: 'o' returned %v, want True", result)
	}

	// Contains substring
	result = vm.Send(s, "primIncludes:", []Value{NewStringValue("wor")})
	if result != True {
		t.Errorf("primIncludes: 'wor' returned %v, want True", result)
	}

	// Does not contain
	result = vm.Send(s, "primIncludes:", []Value{NewStringValue("x")})
	if result != False {
...
func TestStringPrimAsUppercase(t *testing.T) {
	...
	if GetStringContent(result) != "HELLO WORLD" {
		t.Errorf("primAsUppercase returned %q, want %q", GetStringContent(result), "HELLO WORLD")
	}
}
...
func TestStringPrimAsLowercase(t *testing.T) {
	...
	if GetStringContent(result) != "hello world" {
		t.Errorf("primAsLowercase returned %q, want %q", GetStringContent(result), "hello world")
	}
}
...
func TestStringPrimCopyFromTo(t *testing.T) {
	...
	if GetStringContent(result) != "hello" {
		t.Errorf("primCopyFrom:0 to:5 returned %q, want %q", GetStringContent(result), "hello")
	}

	// Copy middle
	result = vm.Send(s, "primCopyFrom:to:", []Value{FromSmallInt(6), FromSmallInt(11)})
	if GetStringContent(result) != "world" {
		t.Errorf("primCopyFrom:6 to:11 returned %q, want %q", GetStringContent(result), "world")
	}

	// End exceeds string length (clamped)
	result = vm.Send(s, "primCopyFrom:to:", []Value{FromSmallInt(6), FromSmallInt(100)})
	if GetStringContent(result) != "world" {
		t.Errorf("primCopyFrom:6 to:100 returned %q, want %q", GetStringContent(result), "world")
	}

	// Start exceeds end
	result = vm.Send(s, "primCopyFrom:to:", []Value{FromSmallInt(5), FromSmallInt(3)})
	if GetStringContent(result) != "" {
		t.Errorf("primCopyFrom:5 to:3 returned %q, want empty string", GetStringContent(result))
	}
}
